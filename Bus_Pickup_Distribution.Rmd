...
setwd('/Users/james/Desktop/MIT_IDSS_Meetings/Transit_Authority_Bus_Data')


MBTA_Bus<-read_csv("MBTA_Bus_Ridership_by_Time_Period_Season_Route_Line_and_Stop_7_17_Data.csv")

print(MBTA_Bus)

plot <- ggplot(MBTA_Bus, aes(x=average_ons)) + 
  geom_density()
print(plot)



AM_Peak<-filter(MBTA_Bus, time_period_name=="AM_PEAK" )
print(AM_Peak)

plot_AM <- ggplot(AM_Peak, aes(x=average_ons)) + 
  geom_density()
print(plot_AM)


table(MBTA_Bus$time_period_name)

PM_Peak<-filter(MBTA_Bus, time_period_name=="PM_PEAK" )
PM_Peak<-filter(MBTA_Bus, season=="Fall 2019" )

print(PM_Peak)

plot_PM <- ggplot(PM_Peak, aes(x=average_ons)) + 
  geom_density()
print(plot_PM)


Route_28<-filter(MBTA_Bus, (route_name==28) )
print(Route_28)
